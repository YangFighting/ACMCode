                                        ...
                                        pf_kdtree_node_t *node, int key[], double value)
{
  ...
  if (node == NULL)
  {
	  //printf("self->node_count:%d, self->node_max_count: %d\n", self->node_count, self->node_max_count);
...
void pf_kdtree_print_node(pf_kdtree_t *self, pf_kdtree_node_t *node)
{
  if (node->leaf)
  {
    printf("(%-2d %-2d %-2d %-2d)\n", node->key[0], node->key[1], node->key[2], node->cluster);
    printf("%*s", node->depth * 11, "");
  }
  else
  {
    //printf("(%+02d %+02d %+02d) ", node->key[0], node->key[1], node->key[2]);
	  printf("(%-4d %-.3f) ", node->pivot_dim, node->pivot_value);
